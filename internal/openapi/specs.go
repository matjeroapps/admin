...
func adminRoutes() []RouteSpec {
	return []RouteSpec{
		...
		},
		{
			...
			OperationID: "listAdminDomains",
			Summary:     "List storefront domains",
			Description: "Lists storefront domains across stores with optional lifecycle status, domain type, store, seller, and search filters.",
			Tags:        []string{"Stores", "Audit"},
			Auth:        true,
			Parameters: []ParameterSpec{
				LimitParam(),
				OffsetParam(),
				StringParam("store_id", "Filter by store identifier", false),
				StringParam("seller_id", "Filter by seller identifier", false),
				StringParam("status", "Filter by domain lifecycle status (pending, verified, active, failed, disabled)", false),
				StringParam("domain_type", "Filter by domain type (platform, custom)", false),
				StringParam("search", "Search domain hostnames", false),
			},
			Responses: ListResponses[coreclient.StoreDomain]("Storefront domain collection"),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/admin/domains/{domain_id}/disable",
			OperationID: "disableAdminDomain",
			Summary:     "Disable a storefront domain",
			Description: "Disables a platform or custom storefront domain. Disabling a primary domain clears is_primary and prompts Core to promote an eligible active platform domain for the store. Core remains the source of truth for lifecycle transitions.",
			Tags:        []string{"Stores", "Audit"},
			Auth:        true,
			Parameters:  []ParameterSpec{PathStringParam("domain_id", "Domain identifier")},
			Responses:   AuthOKResponses("Disabled domain payload", coreclient.StoreDomain{}),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/admin/domains/{domain_id}/enable",
			OperationID: "enableAdminDomain",
			Summary:     "Re-enable a disabled storefront domain",
			Description: "Re-enables a disabled storefront domain. Enable is valid only when current status is disabled. Re-enabling a custom domain returns it to verified (if verified_at != nil) or pending (if verified_at == nil), and does not directly make it active. Re-enabling a platform domain returns it to active.",
			Tags:        []string{"Stores", "Audit"},
			Auth:        true,
			Parameters:  []ParameterSpec{PathStringParam("domain_id", "Domain identifier")},
			Responses:   AuthOKResponses("Re-enabled domain payload", coreclient.StoreDomain{}),
		},
	}
}
